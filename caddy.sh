...
Download_caddy(){
	...
	if [[ ${bit} == "x86_64" ]]; then
		wget --no-check-certificate -O "caddy" "https://github.com/cjiasu/Linux-NetSpeed/raw/master/caddy"
        ...
        #[[ ! -e ${caddy_file} ]] && echo -e "${Error_font_prefix}[错误]${Font_suffix} Caddy 解压 失败或压缩文件错误 !" && exit 1
        #rm -rf LICENSES.txt
        #rm -rf README.txt
        #rm -rf CHANGES.txt
        #rm -rf "init/"
